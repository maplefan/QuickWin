...
VideoForm::VideoForm(QString path , QWidget *parent) :
...
//    setAttribute(Qt::WA_TranslucentBackground);
    ...
    connect(ui->pushButton_open, &QPushButton::clicked, this, &VideoForm::slotBtnClick);
    connect(ui->toolButton_open, &QPushButton::clicked, this, &VideoForm::slotBtnClick);
    connect(ui->pushButton_play, &QPushButton::clicked, this, &VideoForm::slotBtnClick);
    connect(ui->pushButton_pause,&QPushButton::clicked, this, &VideoForm::slotBtnClick);
    connect(ui->pushButton_stop, &QPushButton::clicked, this, &VideoForm::slotBtnClick);
    connect(ui->pushButton_volume, &QPushButton::clicked, this, &VideoForm::slotBtnClick);
    ...
    connect(mTimer, &QTimer::timeout, this, &VideoForm::slotTimerTimeOut);
    mTimer->setInterval(500);

    mTimer_CheckControlWidget = new QTimer; //用于控制控制界面的出现和隐藏
    connect(mTimer_CheckControlWidget, &QTimer::timeout, this, &VideoForm::slotTimerTimeOut);
    ...
    resize(1024,768);
    //setTitle("我的播放器-V%1".arg(AppConfig::VERSION_NAME));
...
//            s.replace("/","\\");
        ...
        mPlayer->startPlay(path.toStdString());

        AppConfig::gVideoFilePath = path;
...
void VideoForm::showOutControlWidget()
{
    IsControlHide = false;
...
void VideoForm::hideControlWidget()
{
    IsControlHide = true;
...
void VideoForm::slotSliderMoved(int value)
{
...
void VideoForm::slotTimerTimeOut()
{
...
void VideoForm::slotBtnClick(bool isChecked)
{
...
///打开文件失败
void VideoForm::onOpenVideoFileFailed(const int &code)
{
...
///打开SDL失败的时候回调此函数
void VideoForm::onOpenSdlFailed(const int &code)
{
    FunctionTransfer::runInMainThread([=]()
    {
...
///获取到视频时长的时候调用此函数
void VideoForm::onTotalTimeChanged(const int64_t &uSec)
{
    FunctionTransfer::runInMainThread([=]()
    {
...
///播放器状态改变的时候回调此函数
void VideoForm::onPlayerStateChanged(const VideoPlayerState &state, const bool &hasVideo, const bool &hasAudio)
{
    FunctionTransfer::runInMainThread([=]()
    {
...
void VideoForm::onDisplayVideo(std::shared_ptr<VideoFrame> videoFrame)
{
    ui->widget_videoPlayer->inputOneFrame(videoFrame);
}

//图片显示部件时间过滤器处理
bool VideoForm::eventFilter(QObject *target, QEvent *event)
{
